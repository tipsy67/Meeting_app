...
async def get_lectures_list(l10n, lectures: list[dict]) -> InlineKeyboardMarkup:
    buttons_data = [
        (lecture.get('name'),
        f'open:lecture:{str(lecture.get('_id'))}:{lecture.get('name')}') for lecture in lectures
    ]
...
def get_lecture_users_list(
    l10n, users: list[dict], name: str, included_users: Optional[set[int]] = None, lecture: str = None
) -> InlineKeyboardMarkup:

    kb1 = create_buttons(None, get_users_button_list(users, name, included_users))
    additional_buttons = [('kb_back', f'open:lecture:{lecture}')]
