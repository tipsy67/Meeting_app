...
async def get_conference_by_id(conference_id: str) -> ConferenceModel | ErrorResponseModel:
    """
...
async def get_user_by_id(user_id: int) -> UserResponse | ErrorResponseModel:
    ...
    user['id'] = user.pop('_id', None) #TODO: сказать Дэну что лучше использовать pydantic model для mongo с alias
