    ...
    if stream is None:
        ...
        if stream_data:
            ...
            await stream_collection.insert_one(
                YoutubeStreamModel.model_dump(new_stream, by_alias=True)
            )
            return new_stream
    ...
    try:
        ...
        if "E11000 duplicate key error" in str(e):
            ...
            return ErrorResponseModel(
                detail="Recording already exists", status_code=400
            )
        print("❌ Unexpected DB error:", type(e), e)
        return ErrorResponseModel(
            detail=f"Unexpected error occurred{e}", status_code=500
        )
...
async def insert_conference(
    ...
    * datetime.now(tz=timezone(timedelta(hours=2)))
    """
    try:
        await conference_collection.insert_one(
            ConferenceModel.model_dump(conference, by_alias=True)
        )
        return conference
    except Exception as e:
        if "E11000 duplicate key error" in str(e):
            print("🔁 Duplicate detected")
            return ErrorResponseModel(
                detail="Conference already exists", status_code=400
            )
        print("❌ Unexpected DB error:", type(e), e)
        return ErrorResponseModel(
            detail=f"Unexpected error occurred{e}", status_code=500
...
async def get_conference(
    ...
    try:
        ...
        if not conference:
            return ErrorResponseModel(detail="Conference not found", status_code=404)
        return ConferenceOutputModel.model_validate(conference)
    except Exception as e:
        print(f"Error: {e}")
        return ErrorResponseModel(
            detail=f"Error retrieving conference: {e}", status_code=500
        )
